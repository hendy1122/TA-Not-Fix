#!/usr/bin/env python3
"""
Enhanced Gas Sensor Array System - COMPLETE FIXED VERSION 4.2
Fixed monitoring data save + Enhanced TGS2602 stuck sensor recovery
Semua menu berfungsi + Auto CSV export + Sensor recovery tools
"""
...
class AdvancedSensitivityManager:
    ...
    def __init__(self, logger):
        ...
        self.sensitivity_profiles = {
            ...
            'TGS2602': {
                'ultra_sensitive': {'multiplier': 15.0, 'threshold': 0.01, 'baseline_factor': 0.98},  # Enhanced for stuck sensor
                'high_sensitive': {'multiplier': 8.0, 'threshold': 0.03, 'baseline_factor': 0.92},
                'normal': {'multiplier': 3.0, 'threshold': 0.06, 'baseline_factor': 0.87},
                'moderate': {'multiplier': 1.5, 'threshold': 0.10, 'baseline_factor': 0.82},
                'conservative': {'multiplier': 0.8, 'threshold': 0.15, 'baseline_factor': 0.77}
            },
            'TGS2610': {
        ...
        self.current_sensitivity = {
            'TGS2600': 'normal',
            'TGS2602': 'ultra_sensitive',  # Default ultra for stuck sensor
            'TGS2610': 'normal'
        }
        ...
        self.custom_factors = {
            'TGS2600': 1.0,
            'TGS2602': 3.0,  # Higher factor for stuck sensor
            'TGS2610': 1.0
        }
    ...
    def advanced_ppm_calculation(self, sensor_name, current_voltage, baseline_voltage=None, gas_type='auto'):
        """Advanced PPM calculation dengan multiple algorithms - Enhanced for stuck sensors"""
        ...
        # Special handling for TGS2602 stuck sensor
        if sensor_name == 'TGS2602':
            # More aggressive detection for stuck sensor
            if abs(voltage_drop) < 0.001:  # Very tiny changes
                ppm_algo1 = voltage_drop * 5000 * profile['multiplier'] * custom_factor
            else:
                ppm_algo1 = voltage_drop * 2000 * profile['multiplier'] * custom_factor
            
            # Enhanced exponential for stuck sensor
            if voltage_drop > 0:
                ppm_algo2 = (math.exp(voltage_drop * 15) - 1) * profile['multiplier'] * custom_factor * 100
            else:
                ppm_algo2 = abs(voltage_drop) * 1000 * profile['multiplier'] * custom_factor
            
            # Gas-specific enhanced
            gas_factor = self.get_gas_response_factor(sensor_name, gas_type) * 2.0  # Double factor
            ppm_algo3 = (abs(voltage_drop) ** 1.2) * 2000 * gas_factor * custom_factor
            
        else:
            # Original algorithm for other sensors
            if abs(voltage_drop) < 0.01:
        ...
        # Combine algorithms
        algorithms = [abs(ppm_algo1), abs(ppm_algo2), abs(ppm_algo3)]
        ...
        # Apply constraints
        max_ppm = 2000 if profile_name in ['ultra_sensitive', 'high_sensitive'] else 1000
        return min(max_ppm, max(0, final_ppm))
    
    def get_gas_response_factor(self, sensor_name, gas_type):
        """Get gas-specific response factor - Enhanced for TGS2602"""
        response_factors = {
            'TGS2600': {
            ...
            'TGS2602': {
                'alcohol': 6.0, 'toluene': 5.5, 'ammonia': 5.0, 'h2s': 7.0,  # Enhanced factors
                'pertalite': 5.2, 'pertamax': 6.0, 'auto': 5.0, 'default': 4.0
            },
            'TGS2610': {
    ...
    def tgs2602_stuck_recovery(self, sensor_array):
        """Special recovery procedure for stuck TGS2602"""
        ...
        voltage_readings = []
        for i in range(20):
            voltage = sensor_array.sensor_config['TGS2602']['channel'].voltage
            voltage_readings.append(voltage)
            time.sleep(0.5)
        
        voltage_mean = np.mean(voltage_readings)
        voltage_std = np.std(voltage_readings)
        
        print(f"Current voltage: {voltage_mean:.3f}V ± {voltage_std:.3f}V")
        
        if voltage_std < 0.001:
            print("❌ CONFIRMED: Sensor appears stuck (no variation)")
            
            print("\nStep 2: Applying recovery techniques...")
            
            # Technique 1: Ultra-sensitive mode
            print("  🔧 Enabling ultra-sensitive mode...")
            self.current_sensitivity['TGS2602'] = 'ultra_sensitive'
            self.custom_factors['TGS2602'] = 5.0
            
            # Technique 2: Enhanced emergency mode
            print("  🚨 Enabling enhanced emergency mode...")
            sensor_array.sensor_config['TGS2602']['use_emergency_ppm'] = True
            sensor_array.sensor_config['TGS2602']['emergency_mode'] = True
            
            # Technique 3: Force baseline adjustment
            print("  ⚖️ Force baseline adjustment...")
            sensor_array.drift_manager.voltage_adjustments['TGS2602']['current'] = voltage_mean
            sensor_array.drift_manager.voltage_adjustments['TGS2602']['adjusted'] = True
            sensor_array.drift_manager.normalization_factors['TGS2602'] = 0.5  # Aggressive normalization
            
            print("\nStep 3: Testing recovery...")
            test_ppm = self.advanced_ppm_calculation('TGS2602', voltage_mean, 1.6)
            emergency_ppm = sensor_array.emergency_ppm_calc.calculate_emergency_ppm(
                'TGS2602', voltage_mean, 'auto', self
            )
            
            print(f"  Advanced PPM: {test_ppm:.1f}")
            print(f"  Emergency PPM: {emergency_ppm:.1f}")
            
            if test_ppm > 0 or emergency_ppm > 0:
                print("✅ RECOVERY SUCCESSFUL - Sensor now responding")
                self.save_sensitivity_data()
                sensor_array.drift_manager.save_drift_data()
                return True
            else:
                print("⚠️ Partial recovery - trying extreme measures...")
                
                # Extreme measure: Force PPM calculation
                extreme_ppm = abs(voltage_mean - 1.6) * 10000  # Force calculation
                print(f"  Extreme PPM: {extreme_ppm:.1f}")
                
                if extreme_ppm > 0:
                    print("✅ EXTREME RECOVERY - Using forced calculation")
                    return True
                else:
                    print("❌ Hardware issue - sensor may need replacement")
                    return False
        else:
            print("✅ Sensor shows variation - may not be stuck")
            return True
    ...
    def save_sensitivity_data(self):
        ...
        sensitivity_data = {
            ...
            'version': 'advanced_sensitivity_v4.2',
    ...
    def load_sensitivity_data(self):
        ...
        try:
            ...
            self.current_sensitivity = data.get('current_sensitivity', {
                'TGS2600': 'normal', 'TGS2602': 'ultra_sensitive', 'TGS2610': 'normal'
            })
            self.custom_factors = data.get('custom_factors', {
                'TGS2600': 1.0, 'TGS2602': 3.0, 'TGS2610': 1.0
            })
...
class EmergencyPPMCalculator:
    ...
    def __init__(self, logger):
        ...
        self.emergency_baselines = {
            ...
            'TGS2602': {
                'clean_air_voltage': 1.6,
                'gas_response_factor': 8.0,  # Enhanced for stuck sensor
                'detection_threshold': 0.001,  # More sensitive
                'voltage_noise_threshold': 0.0005  # Detect tiny changes
            },
            'TGS2610': {
                'clean_air_voltage': 1.6,
    ...
    def calculate_emergency_ppm(self, sensor_name, current_voltage, gas_type='default', sensitivity_manager=None):
        """Enhanced emergency PPM calculation - Special handling for TGS2602"""
        ...
        # For TGS2602, detect even the tiniest changes
        if sensor_name == 'TGS2602':
            if abs(voltage_drop) < baseline['voltage_noise_threshold']:
                # Even if no detectable drop, assume some gas presence
                ppm = abs(current_voltage - 1.6) * 5000
            else:
                ppm = abs(voltage_drop) * 10000  # Very aggressive calculation
        else:
            if abs(voltage_drop) < baseline['voltage_noise_threshold']:
                return 0
        ...
        if sensor_name == 'TGS2602':
            # Special calculation for stuck TGS2602
            if abs(voltage_drop) < 0.01:
                ppm = response_factor * abs(voltage_drop) * 10000
            else:
                ppm = response_factor * (abs(voltage_drop) * 1000) ** 1.5
        else:
        ...
        max_ppm = 5000 if sensor_name == 'TGS2602' else (2000 if sensitivity_multiplier > 5 else 1000)
...
class SmartDriftManager:
    ...
    def smart_daily_drift_check(self, sensor_array):
        ...
        for sensor_name, config in sensor_array.sensor_config.items():
            ...
            drift = abs(current_voltage - original_baseline)
            drift_percentage = (drift / original_baseline) * 100
            
            print(f"\n{sensor_name}:")
            print(f"  Original baseline: {original_baseline:.3f}V")
            print(f"  Current voltage: {current_voltage:.3f}V")
            print(f"  Drift: {drift:.3f}V ({drift_percentage:.1f}%)")
            
            if drift_percentage > 10:
                print(f"  ⚠️ HIGH DRIFT - Applying compensation")
                ...
                print(f"  🔧 MODERATE DRIFT - Applying normalization")
                self.normalization_factors[sensor_name] = original_baseline / current_voltage
            else:
                print(f"  ✅ STABLE")
        
        self.daily_check_done = True
        self.save_drift_data()
        print("\n✅ Daily drift check completed")
    
    def quick_stability_test(self, sensor_array, duration=60):
        """Quick stability test"""
        print(f"\n⚡ QUICK STABILITY TEST - {duration}s")
        print("="*50)
        
        stability_data = {}
        
        for sensor_name in sensor_array.sensor_config.keys():
            readings = []
            print(f"\nTesting {sensor_name} stability...")
            ...
            for i in range(duration):
                ...
                if i % 10 == 0:
                    print(f"  Progress: {i}/{duration}s")
                
                time.sleep(1)
            
            mean_voltage = np.mean(readings)
            std_voltage = np.std(readings)
            stability_score = (1 - std_voltage/mean_voltage) * 100 if mean_voltage > 0 else 0
            
            stability_data[sensor_name] = {
                'mean': mean_voltage,
                'std': std_voltage,
                'stability_score': stability_score
            }
            
            print(f"  Mean: {mean_voltage:.3f}V ± {std_voltage:.3f}V")
            print(f"  Stability: {stability_score:.1f}%")
            
            # Special detection for stuck sensors
            if std_voltage < 0.001:
                print(f"  ⚠️ POSSIBLE STUCK SENSOR - Very low variation")
                if sensor_name == 'TGS2602':
                    print(f"  🩺 Recommend running TGS2602 recovery procedure")
            elif stability_score < 90:
                print(f"  ⚠️ Poor stability - may need calibration")
            else:
    ...
    def smart_drift_status_report(self):
        ...
        print(f"Last calibration: {self.last_calibration_time or 'Never'}")
        print(f"Daily check done: {'Yes' if self.daily_check_done else 'No'}")
        ...
        print(f"\n📈 VOLTAGE ADJUSTMENTS:")
        for sensor, data in self.voltage_adjustments.items():
            status = "ADJUSTED" if data['adjusted'] else "ORIGINAL"
            print(f"  {sensor}: {data['original']:.3f}V → {data['current']:.3f}V ({status})")
    ...
    def manual_drift_compensation_reset(self):
        ...
        print("1. Reset all compensation factors")
        print("2. Reset normalization factors")
        print("3. Reset voltage adjustments")
        print("4. Full reset")
        ...
        if choice == '1':
            self.drift_compensation_factors = {}
            print("✅ Compensation factors reset")
        elif choice == '2':
            self.normalization_factors = {'TGS2600': 1.0, 'TGS2602': 1.0, 'TGS2610': 1.0}
            print("✅ Normalization factors reset")
        elif choice == '3':
            for sensor in self.voltage_adjustments:
                self.voltage_adjustments[sensor]['adjusted'] = False
                self.voltage_adjustments[sensor]['current'] = self.voltage_adjustments[sensor]['original']
            print("✅ Voltage adjustments reset")
        elif choice == '4':
            self.drift_compensation_factors = {}
            self.normalization_factors = {'TGS2600': 1.0, 'TGS2602': 1.0, 'TGS2610': 1.0}
            for sensor in self.voltage_adjustments:
                ...
                self.voltage_adjustments[sensor]['current'] = self.voltage_adjustments[sensor]['original']
            self.daily_check_done = False
            print("✅ Full drift compensation reset")
    ...
    def auto_baseline_reset(self, sensor_array):
        ...
        print("\n🔄 AUTO BASELINE RESET")
        print("="*50)
        print("This will reset baselines to current sensor readings")
        print("Make sure sensors are in CLEAN AIR!")
        ...
        for sensor_name, config in sensor_array.sensor_config.items():
            ...
            self.original_baseline[sensor_name] = current_voltage
            self.current_baseline[sensor_name] = current_voltage
            self.voltage_adjustments[sensor_name]['original'] = current_voltage
            self.voltage_adjustments[sensor_name]['current'] = current_voltage
            self.voltage_adjustments[sensor_name]['adjusted'] = False
            
            # Reset factors
            self.drift_compensation_factors.pop(sensor_name, None)
            self.normalization_factors[sensor_name] = 1.0
            
            print(f"  {sensor_name}: {current_voltage:.3f}V")
        
        self.daily_check_done = False
        self.save_drift_data()
        print("\n✅ Auto baseline reset completed")
    ...
    def smart_system_health_check(self, sensor_array):
        ...
        for sensor_name, config in sensor_array.sensor_config.items():
            ...
            # Voltage check
            voltage_ok = 0.5 <= current_voltage <= 4.5
            
            # Calibration check
            calibration_ok = R0 is not None and R0 > 0
            
            # Drift check
            original_baseline = self.original_baseline.get(sensor_name, 1.6)
            drift = abs(current_voltage - original_baseline)
            drift_percentage = (drift / original_baseline) * 100
            drift_ok = drift_percentage < 10
            
            # Stuck sensor check
            readings = []
            for i in range(5):
                readings.append(config['channel'].voltage)
                time.sleep(0.2)
            voltage_variation = np.std(readings)
            stuck_sensor = voltage_variation < 0.001
            
            # Overall sensor health
            if voltage_ok and calibration_ok and drift_ok and not stuck_sensor:
                sensor_health = "EXCELLENT"
            elif voltage_ok and (calibration_ok or drift_ok) and not stuck_sensor:
                sensor_health = "GOOD"
            ...
            status_icon = "✅" if sensor_health == "EXCELLENT" else "⚠️" if sensor_health == "GOOD" else "🩺" if sensor_health == "STUCK" else "❌"
            print(f"\n{status_icon} {sensor_name}: {sensor_health}")
            print(f"   Voltage: {current_voltage:.3f}V {'✅' if voltage_ok else '❌'}")
            print(f"   Calibration: {'✅' if calibration_ok else '❌'}")
            print(f"   Drift: {drift_percentage:.1f}% {'✅' if drift_ok else '❌'}")
            print(f"   Stuck: {'❌ YES' if stuck_sensor else '✅ NO'}")
            
            if stuck_sensor and sensor_name == 'TGS2602':
                print(f"   🩺 Use menu 29 for TGS2602 recovery")
        
        print(f"\n🏥 OVERALL SYSTEM HEALTH: {overall_health}")
        
        if overall_health == "POOR":
            print("\n🔧 RECOMMENDATIONS:")
            print("- Run sensor calibration")
            print("- Check physical connections")
            print("- For stuck TGS2602: Use recovery procedure (menu 29)")
        elif overall_health == "GOOD":
            print("\n💡 RECOMMENDATIONS:")
            print("- Consider drift compensation")
            print("- Monitor stability over time")
    ...
    def smart_voltage_adjustment(self, sensor_array):
        ...
        for sensor_name, config in sensor_array.sensor_config.items():
            current_voltage = config['channel'].voltage
            original_voltage = self.voltage_adjustments[sensor_name]['original']
            
            voltage_diff = abs(current_voltage - original_voltage)
            
            print(f"\n{sensor_name}:")
            print(f"  Original: {original_voltage:.3f}V")
            print(f"  Current: {current_voltage:.3f}V")
            print(f"  Difference: {voltage_diff:.3f}V")
            
            if voltage_diff > 0.1:
                print(f"  🔧 SIGNIFICANT CHANGE - Applying adjustment")
                self.voltage_adjustments[sensor_name]['current'] = current_voltage
                self.voltage_adjustments[sensor_name]['adjusted'] = True
                
                # Calculate adjustment factor
                adjustment_factor = original_voltage / current_voltage
                self.normalization_factors[sensor_name] = adjustment_factor
    ...
    def sensor_responsivity_check(self, sensor_array):
        ...
        print("\n📡 SENSOR RESPONSIVITY CHECK")
        print("="*50)
        print("This will test how well sensors respond to changes")
        
        responsivity_data = {}
        
        for sensor_name, config in sensor_array.sensor_config.items():
            print(f"\n🧪 Testing {sensor_name} responsivity...")
            ...
            print(f"  Baseline: {baseline_mean:.3f}V ± {baseline_std:.3f}V")
            
            # Test for minor fluctuations (indicator of responsivity)
            fluctuation_score = baseline_std * 1000  # Convert to millivolts
            
            if fluctuation_score > 5:
                responsivity = "HIGH"
            elif fluctuation_score > 2:
                responsivity = "MODERATE"
            elif fluctuation_score > 0.1:
                responsivity = "LOW"
            else:
                responsivity = "STUCK"
            
            responsivity_data[sensor_name] = {
            ...
            print(f"  Fluctuation: {fluctuation_score:.1f}mV")
            print(f"  Responsivity: {responsivity}")
            
            if responsivity == "STUCK" and sensor_name == 'TGS2602':
                print(f"  🩺 TGS2602 appears stuck - recommend recovery procedure")
    ...
    def quick_voltage_check(self, sensor_array):
        ...
        for sensor_name, config in sensor_array.sensor_config.items():
            ...
            print(f"{sensor_name}: {voltage:.3f}V {status}")
    ...
    def enable_ultra_sensitive_mode(self, sensor_array, sensor_name):
        ...
        # Special handling for TGS2602
        if sensor_name == 'TGS2602':
            sensor_array.sensitivity_manager.custom_factors[sensor_name] = 5.0
        else:
    ...
    def save_drift_data(self):
        ...
        drift_data = {
            'timestamp': datetime.now().isoformat(),
            'version': 'smart_drift_v4.2_complete',
...
class DataExportManager:
    """Manager untuk export data monitoring ke CSV"""
    ...
    def add_monitoring_sample(self, readings, predicted_gas, confidence):
        """Add sample to monitoring data"""
        ...
        for sensor_name, data in readings.items():
            sample.update({
                f'{sensor_name}_voltage': data['voltage'],
                f'{sensor_name}_raw_voltage': data['raw_voltage'],
                f'{sensor_name}_resistance': data['resistance'],
                f'{sensor_name}_ppm': data['ppm'],
                f'{sensor_name}_emergency_ppm': data['emergency_ppm'],
                f'{sensor_name}_advanced_ppm': data['advanced_ppm'],
                f'{sensor_name}_mode': data['mode'],
                f'{sensor_name}_drift_factor': data['drift_factor'],
                f'{sensor_name}_normalization_factor': data['normalization_factor'],
                f'{sensor_name}_sensitivity_profile': data['sensitivity_profile'],
                f'{sensor_name}_sensitivity_multiplier': data['sensitivity_multiplier']
            })
        
        self.monitoring_data.append(sample)
        
        # Auto-save if enabled
        if self.auto_save and len(self.monitoring_data) % self.save_interval == 0:
            self.save_monitoring_data()
    
    def save_monitoring_data(self, filename=None):
        """Save monitoring data to CSV"""
        if not self.monitoring_data:
            self.logger.warning("No monitoring data to save")
            return None
        
        if filename is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f"data/monitoring_data_{timestamp}.csv"
        
        # Ensure data directory exists
        Path("data").mkdir(exist_ok=True)
        
        # Get all possible keys from samples
        all_keys = set()
        ...
        try:
            ...
            print(f"📊 Monitoring data saved: {filename} ({len(self.monitoring_data)} samples)")
            return filename
    ...
    def clear_monitoring_data(self):
        """Clear monitoring data buffer"""
        self.monitoring_data.clear()
        self.logger.info("Monitoring data buffer cleared")
    
    def get_monitoring_stats(self):
        """Get statistics about current monitoring data"""
        if not self.monitoring_data:
            return "No monitoring data available"
        
        total_samples = len(self.monitoring_data)
        first_sample = self.monitoring_data[0]['timestamp']
        last_sample = self.monitoring_data[-1]['timestamp']
        
        # Count gas types
        gas_counts = {}
        for sample in self.monitoring_data:
            gas = sample.get('predicted_gas', 'Unknown')
...
📊 MONITORING DATA STATISTICS:
  ...
  Gas type distribution: {gas_counts}
        """
...
class EnhancedDatasheetGasSensorArray:
    def __init__(self):
        ...
        self.sensor_config = {
            ...
            'TGS2602': {
                ...
                'emergency_mode': False,
                'use_emergency_ppm': True,  # Default emergency mode for stuck sensor
                'use_advanced_sensitivity': True
            },
            'TGS2610': {
        ...
        self.data_export_manager = DataExportManager(self.logger)
        ...
        self.logger.info("Enhanced Gas Sensor Array System v4.2 - TGS2602 Recovery + Auto CSV Export initialized")
    ...
    def calibrate_sensors(self, duration=300):
        ...
        for sensor_name in self.sensor_config.keys():
            ...
            # Special handling for TGS2602
            if sensor_name == 'TGS2602':
                self.sensor_config[sensor_name]['use_emergency_ppm'] = True  # Keep emergency mode
            else:
            ...
            calibration_results[sensor_name] = {
                ...
                'stability': (voltage_std / voltage_mean) * 100 if voltage_mean > 0 else 0
            }
        ...
        try:
            ...
            for sensor_name, data in calib_data['sensors'].items():
                if sensor_name in self.sensor_config:
                    ...
                    # Special handling for TGS2602
                    if sensor_name == 'TGS2602':
                        self.sensor_config[sensor_name]['use_emergency_ppm'] = True
                    else:
                        self.sensor_config[sensor_name]['use_emergency_ppm'] = False
    ...
    def collect_training_data(self, duration=300, gas_type='unknown', concentration=0):
        """Collect training data with gas type and concentration"""
        ...
        # Prepare data file
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        data_file = f"data/training_data_{gas_type}_{timestamp}.csv"
        
        # Define headers
        headers = ['timestamp', 'gas_type', 'concentration', 'temperature', 'humidity']
        for sensor in ['TGS2600', 'TGS2602', 'TGS2610']:
            headers.extend([
                f'{sensor}_voltage', f'{sensor}_resistance', f'{sensor}_ppm',
                f'{sensor}_emergency_ppm', f'{sensor}_advanced_ppm',
                f'{sensor}_drift_factor', f'{sensor}_normalization_factor'
            ])
        ...
        with open(data_file, 'w', newline='') as f:
            ...
            while time.time() - start_time < duration:
                ...
                for sensor in ['TGS2600', 'TGS2602', 'TGS2610']:
                    ...
                    row.extend([
                        data['voltage'], data['resistance'], data['ppm'],
                        data['emergency_ppm'], data['advanced_ppm'],
                        data['drift_factor'], data['normalization_factor']
                    ])
                ...
                if sample_count % 10 == 0:
                    print(f"Samples collected: {sample_count} | Remaining: {remaining}s")
    ...
    def train_model(self):
        ...
        data_dir = Path("data")
        data_files = list(data_dir.glob("training_data_*.csv"))
        ...
        for sensor in ['TGS2600', 'TGS2602', 'TGS2610']:
            feature_columns.extend([
                f'{sensor}_voltage', f'{sensor}_ppm', f'{sensor}_emergency_ppm',
                f'{sensor}_advanced_ppm', f'{sensor}_drift_factor', f'{sensor}_normalization_factor'
            ])
        ...
        print(f"Gas types: {y.unique()}")
        
        if len(y.unique()) < 2:
            print("❌ Need at least 2 different gas types for training!")
            return False
    ...
    def start_monitoring(self, mode='datasheet', duration=None):
        """Start monitoring with specified mode - WITH AUTO CSV EXPORT"""
        self.set_sensor_mode(mode)
        
        print(f"\n🚀 STARTING GAS MONITORING - {mode.upper()} MODE")
        print("="*60)
        print("📊 Auto-save to CSV: ENABLED")
        ...
        self.data_export_manager.clear_monitoring_data()
        ...
        try:
            ...
            while True:
                ...
                # Add sample to data export manager
                self.data_export_manager.add_monitoring_sample(readings, predicted_gas, confidence)
                
                print(f"\n⏰ Sample #{sample_count + 1} - {datetime.now().strftime('%H:%M:%S')}")
                print(f"🔍 Predicted Gas: {predicted_gas} (Confidence: {confidence:.3f})")
                
                for sensor, data in readings.items():
                    ppm_display = f"{data['ppm']:.1f}"
                    if data['emergency_mode']:
                        ppm_display = f"{data['emergency_ppm']:.1f} (Emergency)"
                    elif data['advanced_ppm'] > data['ppm']:
                        ppm_display = f"{data['advanced_ppm']:.1f} (Advanced)"
                    
                    print(f"{sensor}: {data['voltage']:.3f}V | {ppm_display} PPM | {data['mode']}")
                
                sample_count += 1
                
                # Show save status every 10 samples
                if sample_count % 10 == 0:
                    print(f"📊 {sample_count} samples collected (auto-saving...)")
        ...
        # Final save
        saved_file = self.data_export_manager.save_monitoring_data()
        if saved_file:
            print(f"✅ Final monitoring data saved: {saved_file}")
            print(self.data_export_manager.get_monitoring_stats())
    ...
    def view_sensor_diagnostics(self):
        """View comprehensive sensor diagnostics"""
        print("\n🔍 COMPREHENSIVE SENSOR DIAGNOSTICS")
        print("="*70)
        
        readings = self.read_sensors()
        
        print(f"🌡️ Environmental: {self.current_temperature}°C, {self.current_humidity}%RH")
        print(f"🔧 Temp Compensation: {'ON' if self.temp_compensation_enabled else 'OFF'}")
        print(f"💧 Humidity Compensation: {'ON' if self.humidity_compensation_enabled else 'OFF'}")
        
        for sensor_name, data in readings.items():
            config = self.sensor_config[sensor_name]
            
            print(f"\n🔍 {sensor_name} DETAILED DIAGNOSTICS:")
            ...
            print(f"     Advanced Mode: {'ON' if data['advanced_sensitivity_mode'] else 'OFF'}")
            print(f"     Emergency Mode: {'ON' if data['emergency_mode'] else 'OFF'}")
            
            print(f"\n   🔧 COMPENSATION STATUS:")
            print(f"     Smart Compensation: {'APPLIED' if data['smart_compensation_applied'] else 'NONE'}")
            print(f"     Drift Factor: {data['drift_factor']:.3f}x")
            print(f"     Normalization Factor: {data['normalization_factor']:.3f}x")
            print(f"     Voltage Adjusted: {'YES' if data['voltage_adjusted'] else 'NO'}")
        
        # Overall system status
        drift_status = self.drift_manager.get_smart_status()
        
        print(f"\n🏥 OVERALL SYSTEM STATUS:")
        print(f"   Drift Manager: {drift_status['overall_health']}")
        print(f"   ML Model: {'LOADED' if self.is_model_trained else 'NOT LOADED'}")
        print(f"   Daily Check: {'DONE' if not self.drift_manager.is_daily_check_needed() else 'NEEDED'}")
        print(f"   Data Export: {'ENABLED' if self.data_export_manager.auto_save else 'DISABLED'}")

def main():
    """Complete main function with all features - FIXED VERSION 4.2"""
    ...
    if not calibration_loaded:
        ...
        print("TGS2602 Emergency Mode: ENABLED by default")
    ...
    while True:
        print("\n" + "="*70)
        print("🧠 SMART Gas Sensor Array System - Complete v4.2 FIXED")
        print("TGS2602 Recovery + Auto CSV Export + Enhanced PPM Response")
        print("="*70)
        print("1. Calibrate sensors")
        print("2. Collect training data")
        print("3. Train machine learning model")
        print("4. Start monitoring - Datasheet mode (AUTO CSV)")
        print("5. Start monitoring - Extended mode (AUTO CSV)")
        ...
        try:
            choice = input("Select option (1-30): ").strip()
            ...
            if choice == '1':
                ...
                gas_type = input("Gas type (e.g., alcohol, pertalite, ammonia): ").strip() or 'unknown'
                concentration = float(input("Approximate concentration (PPM, 0 if unknown): ") or 0)
                
                gas_sensor.collect_training_data(duration, gas_type, concentration)

            elif choice == '3':
                gas_sensor.train_model()

            elif choice == '4':
                duration_input = input("Monitoring duration (seconds, Enter for unlimited): ").strip()
                duration = int(duration_input) if duration_input else None
                gas_sensor.start_monitoring('datasheet', duration)

            elif choice == '5':
                duration_input = input("Monitoring duration (seconds, Enter for unlimited): ").strip()
                duration = int(duration_input) if duration_input else None
                gas_sensor.start_monitoring('extended', duration)
                ...
                temp_input = input("Temperature (°C, current: {:.1f}): ".format(gas_sensor.current_temperature)).strip()
                humidity_input = input("Humidity (%RH, current: {:.1f}): ".format(gas_sensor.current_humidity)).strip()
                
                temperature = float(temp_input) if temp_input else None
                humidity = float(humidity_input) if humidity_input else None
                
                gas_sensor.set_environmental_conditions(temperature, humidity)

            elif choice == '8':
                print("\n🔧 SWITCH SENSOR CALCULATION MODE")
                print("1. Datasheet mode (accurate)")
                print("2. Extended mode (training)")
                ...
                gas_sensor.view_sensor_diagnostics()
                ...
                print(f"\nSelect sensor:")
                for i, sensor in enumerate(sensors, 1):
                    current_status = gas_sensor.sensor_config[sensor].get('use_emergency_ppm', False)
                    status_text = "ON" if current_status else "OFF"
                    print(f"{i}. {sensor} (Currently: {status_text})")
                
                sensor_choice = input("Enter choice (1-3): ").strip()
                if sensor_choice in ['1', '2', '3']:
                    target_sensor = sensors[int(sensor_choice) - 1]
                    current_status = gas_sensor.sensor_config[target_sensor].get('use_emergency_ppm', False)
                    gas_sensor.sensor_config[target_sensor]['use_emergency_ppm'] = not current_status
                    new_status = "ON" if not current_status else "OFF"
                    print(f"✅ {target_sensor} Emergency PPM Mode: {new_status}")
                ...
                if sensor_choice in ['1', '2', '3']:
                    target_sensor = sensors[int(sensor_choice) - 1]
                    voltage = gas_sensor.sensor_config[target_sensor]['channel'].voltage
                    emergency_ppm = gas_sensor.emergency_ppm_calc.calculate_emergency_ppm(
                        target_sensor, voltage, 'auto', gas_sensor.sensitivity_manager
                    )
                    print(f"Emergency PPM for {target_sensor}: {emergency_ppm:.1f}")
            ...
            # NEW TGS2602 RECOVERY FEATURE
            elif choice == '29':
                print("\n🩺 TGS2602 STUCK SENSOR RECOVERY")
                confirm = input("Run TGS2602 recovery procedure? (y/n): ").lower()
                if confirm == 'y':
                    success = gas_sensor.sensitivity_manager.tgs2602_stuck_recovery(gas_sensor)
                    if success:
                        print("\n✅ TGS2602 recovery completed!")
                        print("Test the sensor with gas to verify recovery")
                    else:
                        print("\n❌ Recovery failed - hardware issue suspected")

            elif choice == '30':
                print(gas_sensor.data_export_manager.get_monitoring_stats())
                ...
                # Final save monitoring data if any
                if gas_sensor.data_export_manager.monitoring_data:
                    final_file = gas_sensor.data_export_manager.save_monitoring_data()
                    if final_file:
                        print(f"📊 Final monitoring data saved: {final_file}")
