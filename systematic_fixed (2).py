...
def main():
    ...
    while True:
        ...
        print("31. Reset systematic collection (ALL or SPECIFIC)")
        print("32. ⭐ SELECTIVE SYSTEMATIC COLLECTION")
        print("-"*70)

        try:
            choice = input("Select option (1-32): ").strip()
            ...
            # ADVANCED SENSITIVITY FEATURES (21-25)
                ...
                print("Reset options:")
                print("1. Reset ALL systematic files")
                print("2. Reset SPECIFIC gas types")
                print("3. Cancel")
                
                reset_choice = input("Select option (1-3): ").strip()
                ...
                if reset_choice == '1':
                    ...
                    if confirm == 'RESET':
                        ...
                        print(f"\n✅ All systematic collection reset completed")
                        print(f"💡 You can now start fresh systematic collection")
                    else:
                        print("❌ Reset cancelled")
                
                elif reset_choice == '2':
                    # Reset specific gas types
                    print("\n🎯 RESET SPECIFIC GAS TYPES")
                    print("="*40)
                    
                    # Show available gas files
                    systematic_files = glob.glob("systematic_*.csv")
                    if not systematic_files:
                        print("❌ No systematic files found")
                        continue
                    ...
                    for file in systematic_files:
                        try:
                            df = pd.read_csv(file)
                            gas_type = df['gas_type'].iloc[0]
                            gas_files[gas_type] = file
                        except:
                            continue
                    
                    if not gas_files:
                        print("❌ No valid systematic files found")
                        continue
                    
                    print("Available gas types to reset:")
                    gas_list = list(gas_files.keys())
                    for i, gas_type in enumerate(gas_list, 1):
                        samples = len(pd.read_csv(gas_files[gas_type]))
                        print(f"{i}. {gas_type} ({samples} samples)")
                    
                    print(f"{len(gas_list) + 1}. Select multiple")
                    
                    selection = input(f"Select gas to reset (1-{len(gas_list) + 1}): ").strip()
                    
                    if selection.isdigit() and 1 <= int(selection) <= len(gas_list):
                        # Single gas selection
                        selected_gas = gas_list[int(selection) - 1]
                        file_to_delete = gas_files[selected_gas]
                        
                        confirm = input(f"Delete {selected_gas} data? (y/n): ").lower()
                        if confirm == 'y':
                            try:
                                os.remove(file_to_delete)
                                print(f"✅ Deleted: {selected_gas} data")
                                
                                # Reset status for this gas
                                if selected_gas in gas_sensor.systematic_guide.collection_status:
                                    del gas_sensor.systematic_guide.collection_status[selected_gas]
                                if selected_gas in gas_sensor.systematic_guide.validation_results:
                                    del gas_sensor.systematic_guide.validation_results[selected_gas]
                                
                                print(f"💡 You can now recollect {selected_gas} using Option 2")
                            except Exception as e:
                                print(f"❌ Error deleting file: {e}")
                    
                    elif int(selection) == len(gas_list) + 1:
                        # Multiple selection
                        print(f"\nEnter gas numbers to reset (comma separated):")
                        print(f"Example: 1,3,5 to reset first, third, and fifth gas")
                        
                        multi_selection = input("Gas numbers: ").strip()
                        try:
                            gas_indices = [int(x.strip()) - 1 for x in multi_selection.split(',')]
                            gases_to_delete = [gas_list[i] for i in gas_indices if 0 <= i < len(gas_list)]
                            
                            if gases_to_delete:
                                print(f"\nWill delete: {', '.join(gases_to_delete)}")
                                confirm = input("Confirm deletion? (y/n): ").lower()
                                
                                if confirm == 'y':
                                    for gas_type in gases_to_delete:
                                        try:
                                            os.remove(gas_files[gas_type])
                                            print(f"✅ Deleted: {gas_type}")
                                            
                                            # Reset status
                                            if gas_type in gas_sensor.systematic_guide.collection_status:
                                                del gas_sensor.systematic_guide.collection_status[gas_type]
                                            if gas_type in gas_sensor.systematic_guide.validation_results:
                                                del gas_sensor.systematic_guide.validation_results[gas_type]
                                        except Exception as e:
                                            print(f"❌ Error deleting {gas_type}: {e}")
                                    
                                    print(f"\n💡 You can now recollect deleted gases using:")
                                    print(f"   - Option 2: Individual collection")
                                    print(f"   - Option 26: Complete systematic collection")
                            else:
                                print("❌ No valid gas selection")
                        except:
                ...
                print("\n🎯 SELECTIVE SYSTEMATIC COLLECTION")
                print("="*50)
                print("Choose which gases to collect/recollect")
                ...
                for file in systematic_files:
                    try:
                        df = pd.read_csv(file)
                        gas_type = df['gas_type'].iloc[0]
                        existing_gases.add(gas_type)
                    except:
                        continue
                
                # Available gas types
                all_gases = ['normal', 'alcohol', 'pertalite', 'toluene', 'ammonia', 'butane']
                
                print(f"\n📊 Current Status:")
                for gas in all_gases:
                    if gas in existing_gases:
                        # Get sample count
                        gas_file = next((f for f in systematic_files if gas in f), None)
                        if gas_file:
                            try:
                                samples = len(pd.read_csv(gas_file))
                                print(f"  ✅ {gas}: {samples} samples")
                            except:
                                print(f"  ⚠️ {gas}: file error")
                    else:
                        print(f"  ❌ {gas}: not collected")
                
                print(f"\n🎯 Select gases to collect/recollect:")
                for i, gas in enumerate(all_gases, 1):
                    status = "✅ EXISTS" if gas in existing_gases else "❌ MISSING"
                    print(f"{i}. {gas} ({status})")
                print("7. All missing gases")
                print("8. All gases (recollect everything)")
                
                selection = input("Enter gas numbers (comma separated, e.g., 1,3,5): ").strip()
                
                try:
                    if selection == '7':
                        # Collect only missing gases
                        gases_to_collect = [gas for gas in all_gases if gas not in existing_gases]
                    elif selection == '8':
                        # Collect all gases
                        gases_to_collect = all_gases
                    else:
                        # Parse individual selections
                        gas_indices = [int(x.strip()) - 1 for x in selection.split(',')]
                        gases_to_collect = [all_gases[i] for i in gas_indices if 0 <= i < len(all_gases)]
                    
                    if not gases_to_collect:
                        print("❌ No gases selected")
                        continue
                    
                    print(f"\n🎯 Will collect: {', '.join(gases_to_collect)}")
                    confirm = input("Proceed with selective collection? (y/n): ").lower()
                    
                    if confirm == 'y':
                        # Start selective collection
                        for gas_name in gases_to_collect:
                            print(f"\n" + "="*50)
                            print(f"🎯 COLLECTING: {gas_name.upper()}")
                            print("="*50)
                            ...
                            if gas_info:
                                ...
                                # Ask if ready
                                ready = input(f"Ready to collect {gas_name}? (y/s/q - Yes/Skip/Quit): ").lower()
                                ...
                                if ready == 'y':
                                    ...
                                    # Validate
                                    if gas_sensor.systematic_guide.validate_gas_collection(result, gas_info):
                                        print(f"✅ {gas_name} collection: SUCCESSFUL")
                                        gas_sensor.systematic_guide.collection_status[gas_name] = 'success'
                                    else:
                                        print(f"❌ {gas_name} collection: NEEDS IMPROVEMENT")
                                        retry = input(f"Retry {gas_name}? (y/n): ").lower()
                                        if retry == 'y':
                                            result = gas_sensor.systematic_guide.collect_gas_systematic(gas_sensor, gas_info)
                                            if gas_sensor.systematic_guide.validate_gas_collection(result, gas_info):
                                                print(f"✅ {gas_name} retry: SUCCESSFUL")
                                                gas_sensor.systematic_guide.collection_status[gas_name] = 'success'
                                    
                                    # Recovery time (if not last gas)
                                    if gas_name != gases_to_collect[-1]:
                                        print(f"\n⏳ Recovery time: 5 minutes...")
                                        gas_sensor.systematic_guide.countdown_timer(300, "Recovery")
                                
                                elif ready == 's':
                                    print(f"⏭️ Skipped {gas_name}")
                                    continue
                                elif ready == 'q':
                                    print(f"⏹️ Collection stopped")
                                    break
                            else:
                                print(f"❌ Gas info not found for {gas_name}")
                        
                        print(f"\n🎉 SELECTIVE COLLECTION COMPLETED!")
                        
                        # Offer training
                        train_now = input(f"\nRetrain model with updated data? (y/n): ").lower()
                        if train_now == 'y':
                            success = gas_sensor.systematic_trainer.train_systematic_model()
                            if success:
                                gas_sensor.load_systematic_model()
                    ...
                    print(f"❌ Error in selective collection: {e}")
