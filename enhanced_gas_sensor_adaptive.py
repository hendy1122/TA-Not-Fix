#!/usr/bin/env python3
"""
Enhanced Gas Sensor Array System with Advanced Adaptive Learning & Drift Compensation
Solusi untuk mengatasi masalah voltage drift tanpa perlu collecting data ulang
Version: 3.0 with Adaptive Learning & Smart Drift Management
"""
...
class DriftCompensationManager:
    """Enhanced Drift Compensation Manager dengan adaptive learning"""
...
class EnhancedDatasheetGasSensorArrayV3:
    """Enhanced Gas Sensor Array with Advanced Adaptive Learning v3.0"""
    
    def __init__(self):
        """Initialize enhanced gas sensor array system with adaptive learning"""
        # Setup logging
        logging.basicConfig(
            level=logging.INFO,
        ...
        self.logger.info("Enhanced Gas Sensor Array System v3.0 with Adaptive Learning initialized")
    ...
    def read_sensors(self):
        """Enhanced sensor reading with drift compensation and adaptive learning"""
        readings = {}
    ...
    def calibrate_sensors(self, duration=600):
        """Enhanced calibration with drift compensation integration"""
    ...
    def collect_training_data(self, gas_type, duration=300):
        """Enhanced training data collection with drift compensation"""
        # Auto-switch to extended mode for training
        self.set_sensor_mode('extended')

        self.logger.info(f"Collecting enhanced training data for {gas_type} with drift compensation")
        ...
        while time.time() - start_time < duration:
            ...
            # Enhanced data row with drift compensation info
            data_row = {
                'timestamp': timestamp,
                'gas_type': gas_type,
                'temperature': self.current_temperature,
    ...
    def train_model(self):
        """Enhanced model training with drift compensation features"""
        self.logger.info("Training enhanced ML model with drift compensation features...")
        ...
        # Enhanced feature selection (include drift features)
        feature_columns = [
        ...
        model_metadata = {
            ...
            'model_type': 'adaptive_drift_compensated',
        ...
        self.logger.info("Enhanced adaptive drift-compensated model trained and saved successfully")
    ...
    def predict_gas(self, readings):
        """Enhanced gas prediction with drift compensation and adaptive learning"""
    ...
    def continuous_monitoring_with_adaptive_learning(self, duration=None, monitoring_mode='datasheet'):
        """Enhanced continuous monitoring with adaptive learning"""
        # Set monitoring mode
        self.set_sensor_mode(monitoring_mode)

        self.logger.info(f"Starting adaptive monitoring in {monitoring_mode.upper()} mode...")
        ...
        fieldnames = [
            ...
            'TGS2610_rs_r0_ratio', 'TGS2610_ppm', 'TGS2610_drift_factor',
            'predicted_gas', 'confidence', 'auto_baseline_adjusted'
        ]
        ...
        with open(monitoring_file, 'w', newline='') as csvfile:
            ...
            try:
                while self.is_collecting:
                    ...
                    auto_adjusted = self.adaptive_manager.auto_baseline_adjustment(self, readings)

                    # Track low confidence predictions
                    if confidence < self.adaptive_manager.model_confidence_threshold:
                        low_confidence_count += 1

                    # Enhanced data row with adaptive features
                    data_row = {
                        'timestamp': timestamp,
                        'temperature': self.current_temperature,
                        'humidity': self.current_humidity,
                    ...
                    if confidence < 0.5:
                        print(f"\n⚠️  LOW CONFIDENCE: {predicted_gas} ({confidence:.2f}) - Consider model adaptation")
                    elif confidence < 0.7 and low_confidence_count > 10:
                        print(f"\n🤖 ADAPTIVE ALERT: Frequent low confidence - Model adaptation recommended")
        ...
        low_confidence_rate = low_confidence_count / sample_count if sample_count > 0 else 0
        ...
        if low_confidence_rate > 0.3:
            print("⚠️  High low-confidence rate detected - Model adaptation recommended")
        elif low_confidence_rate > 0.1:
            print("⚠️  Moderate low-confidence rate - Monitor system performance")
        else:
            print("✅ Good confidence rate - System performing well")

        self.logger.info(f"Enhanced adaptive monitoring data saved to {monitoring_file}")
...
def main():
    """Enhanced main function with adaptive learning workflow"""
    gas_sensor = EnhancedDatasheetGasSensorArrayV3()
    ...
    if gas_sensor.drift_manager.is_daily_check_needed():
        print("\n" + "="*70)
        print("🧠 INTELLIGENT DRIFT CHECK RECOMMENDED")
        print("Advanced system detected potential sensor drift.")
        print("The system can automatically compensate and adapt.")
        print("="*70)
        
        response = input("Run intelligent drift check with adaptive learning? (y/n): ").lower()
        if response == 'y':
            gas_sensor.drift_manager.intelligent_drift_compensation(gas_sensor, gas_sensor.adaptive_manager)

    while True:
        print("\n" + "="*80)
        print("🚀 ENHANCED Gas Sensor Array System v3.0 - ADAPTIVE LEARNING")
        print("Advanced Drift Compensation + Intelligent Model Adaptation")
        print("="*80)
        print("1. Calibrate sensors (Enhanced R0 determination - 10 min)")
        print("2. Collect training data (Optimal 5 min timing)")
        print("3. Train machine learning model (With adaptive features)")
        print("4. Start adaptive monitoring - Datasheet mode")
        print("5. Start adaptive monitoring - Extended mode")
        print("6. Test single reading (Detailed adaptive analysis)")
        ...
        print("11. Intelligent drift check & compensation")
        print("12. Smart model drift detection")
        print("13. Quick model adaptation (NO data collection needed!)")
        print("14. Auto baseline adjustment")
        print("15. Comprehensive system health report")
        print("16. Reset all adaptive learning data")
        print("-"*80)
        ...
        try:
            ...
            if choice == '1':
                ...
                print("Note: New calibration will reset adaptive learning data.")
                ...
                print("✅ Optimal timing: 5 minutes for all gas types")
                ...
                print("🤖 Training adaptive model with intelligent features...")
                if gas_sensor.train_model():
                    print("✅ Enhanced adaptive model training completed successfully!")
                    print("Model now includes advanced drift correction and adaptive learning.")
                else:
                ...
                print("🎯 Adaptive monitoring in DATASHEET mode with intelligent features")
                gas_sensor.continuous_monitoring_with_adaptive_learning(duration, 'datasheet')

            elif choice == '5':
                duration_input = input("Monitoring duration (seconds, Enter for infinite): ").strip()
                duration = int(duration_input) if duration_input else None

                print("📊 Adaptive monitoring in EXTENDED mode with intelligent features")
                gas_sensor.continuous_monitoring_with_adaptive_learning(duration, 'extended')
                ...
                print("\n" + "="*70)
                print("🔍 DETAILED ADAPTIVE SENSOR ANALYSIS")
                print("="*70)
                ...
                if confidence < 0.5:
                    print("  ⚠️  Low confidence - Quick model adaptation recommended")
                elif confidence < 0.7:
                    print("  ⚠️  Medium confidence - Monitor system performance")
                else:
                    print("  ✅ High confidence - System performing excellently")

                # Check if clean air for auto adjustment
                is_clean = gas_sensor.adaptive_manager.detect_clean_air_environment(readings)
                if is_clean:
                    print("  🔄 Clean air detected - Auto baseline adjustment available")
                ...
                print("\n" + "="*70)
                print("🔍 COMPREHENSIVE ADAPTIVE SENSOR DIAGNOSTICS")
                print("="*70)
                ...
                if gas_sensor.adaptive_manager.adaptation_history:
                    latest = gas_sensor.adaptive_manager.adaptation_history[-1]
                    print(f"  Last Adaptation: {latest['timestamp']}")
                    print(f"  Adaptation Type: {latest['adaptation_type']}")
                    print(f"  Accuracy Improvement: {latest['accuracy_before']:.3f} → {latest['accuracy_after']:.3f}")
                else:
                    print("  No adaptations performed yet")

            elif choice == '11':
                print("\n🧠 INTELLIGENT DRIFT CHECK & COMPENSATION")
                print("Advanced system will:")
                print("  1. Analyze model performance and confidence patterns")
                print("  2. Detect sensor drift using AI algorithms")
                print("  3. Apply intelligent compensation")
                print("  4. Recommend model adaptation if needed")
                
                confirm = input("\nProceed with intelligent drift analysis? (y/n): ").lower()
                if confirm == 'y':
                    drift_detected, reasons = gas_sensor.drift_manager.smart_drift_detection(gas_sensor, gas_sensor.adaptive_manager)
                    
                    if drift_detected:
                        print(f"\n⚠️  SMART DRIFT DETECTED: {'; '.join(reasons)}")
                        response = input("Apply intelligent compensation? (y/n): ").lower()
                        if response == 'y':
                            gas_sensor.drift_manager.intelligent_drift_compensation(gas_sensor, gas_sensor.adaptive_manager)
                    else:
                        print("\n✅ No significant drift detected - System performing optimally")

            elif choice == '12':
                print("\n🔍 SMART MODEL DRIFT DETECTION")
                print("Analyzing model performance...")
                
                adaptation_needed, reasons, predictions = gas_sensor.adaptive_manager.detect_model_drift(gas_sensor, 20)
                
                if adaptation_needed:
                    print(f"\n⚠️  MODEL ADAPTATION RECOMMENDED")
                    print(f"Reasons: {'; '.join(reasons)}")
                    response = input("Proceed to quick model adaptation? (y/n): ").lower()
                    if response == 'y':
                        gas_type = input("Current gas environment (or 'normal' for clean air): ").strip()
                        gas_sensor.adaptive_manager.quick_model_adaptation(gas_sensor, gas_type, 25)
                else:
                    print("\n✅ Model performing well - No adaptation needed")

            elif choice == '13':
                print("\n🚀 QUICK MODEL ADAPTATION")
                print("Adapt model to current conditions WITHOUT full data collection!")
                print("This will improve accuracy for current environment.")
                ...
                print(f"\n🔄 Adapting model for {gas_type} with {samples} samples...")
                ...
                if success:
                    print("✅ QUICK ADAPTATION COMPLETED!")
                    print("Model has been updated for current conditions.")
                    print("You can now continue monitoring with improved accuracy.")
                else:
                    print("❌ Adaptation failed - check system status")

            elif choice == '14':
                print("\n🔄 AUTO BASELINE ADJUSTMENT")
                print("Checking current environment for automatic baseline adjustment...")
                
                readings = gas_sensor.read_sensors()
                adjusted = gas_sensor.adaptive_manager.auto_baseline_adjustment(gas_sensor, readings)
                
                if adjusted:
                    print("✅ Automatic baseline adjustment applied!")
                    print("Sensor baselines have been updated for better accuracy.")
                else:
                    print("⚠️  No adjustment applied")
                    print("Ensure sensors are in clean air environment for auto adjustment.")

            elif choice == '15':
                print("\n📊 COMPREHENSIVE SYSTEM HEALTH REPORT")
                print("Generating detailed adaptive system analysis...")
                ...
                drift_status = gas_sensor.drift_manager.get_compensation_status()
                if drift_status:
                    print("\n🔧 DRIFT COMPENSATION STATUS:")
                    for sensor, status in drift_status.items():
                        print(f"  {sensor}: {status['compensation_percent']:.1f}% ({status['status']})")
                else:
                    print("\n✅ No drift compensation active - sensors stable")
                
                # Adaptive learning status
                print(f"\n🤖 ADAPTIVE LEARNING STATUS:")
                if gas_sensor.adaptive_manager.adaptation_history:
                    print(f"Total adaptations: {len(gas_sensor.adaptive_manager.adaptation_history)}")
                    latest = gas_sensor.adaptive_manager.adaptation_history[-1]
                    print(f"Last adaptation: {latest['timestamp']}")
                    print(f"Latest accuracy: {latest['accuracy_after']:.3f}")
                else:
                    print("No adaptations performed - using original model")
                ...
                if confidence < 0.6:
                    print("  ⚠️  Low confidence - Quick model adaptation recommended")
                if any(status['status'] == 'HIGH' for status in drift_status.values()):
                    print("  ⚠️  High drift detected - Consider recalibration")
                if is_clean:
                    print("  🔄 Clean air detected - Auto baseline adjustment available")
                if not drift_status and confidence > 0.8:
                    print("  ✅ System performing excellently - no action needed")

            elif choice == '16':
                print("\n🔄 RESET ALL ADAPTIVE LEARNING DATA")
                print("This will:")
                print("  1. Clear all drift compensation factors")
                print("  2. Reset adaptation history")
                print("  3. Remove baseline adjustments")
                print("  4. Force fresh calibration on next run")
                print("\n⚠️  WARNING: This will reset ALL adaptive improvements!")
                ...
                if confirm == 'RESET':
                    ...
                    # Reset adaptive manager
                    gas_sensor.adaptive_manager.adaptation_history = []
                    gas_sensor.adaptive_manager.baseline_shift_patterns = {}
                    gas_sensor.adaptive_manager.last_adaptation = None
                    
                    # Save reset state
                    gas_sensor.drift_manager.save_drift_data()
                    gas_sensor.adaptive_manager.save_adaptation_data()
                    
                    print("✅ ALL ADAPTIVE LEARNING DATA RESET!")
                    print("Run calibration and collect new training data for fresh start.")
                else:
                    print("Reset cancelled.")

            elif choice == '10':
                print("👋 Exiting enhanced adaptive gas sensor system...")
                # Save all adaptive data before exit
                gas_sensor.drift_manager.save_drift_data()
                gas_sensor.adaptive_manager.save_adaptation_data()
                break
