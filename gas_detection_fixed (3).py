...
class GasDetectionSystem:
    ...
    def load_and_train_model(self):
        ...
        try:
            ...
            # First, let's read the raw content to understand the structure
            with open(self.dataset_path, 'r', encoding='utf-8') as file:
                first_line = file.readline().strip()
                self.logger.info(f"First line of CSV: {first_line}")
            
            # Read CSV - try different approaches based on the structure
            try:
                # Method 1: Try reading with semicolon separator and handle quotes
                df = pd.read_csv(self.dataset_path, sep=';', quotechar='"')
                
                # If the first method results in a single column, try alternative parsing
                if len(df.columns) == 1:
                    # Method 2: Read line by line and parse manually
                    data_rows = []
                    with open(self.dataset_path, 'r', encoding='utf-8') as file:
                    ...
                    if lines:
                        ...
                        self.logger.info(f"Parsed headers: {headers}")
                ...
                self.logger.error(f"Error reading CSV with semicolon separator: {e}")
            ...
            self.logger.info(f"Available columns after cleaning: {df.columns.tolist()}")
