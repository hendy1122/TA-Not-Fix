#!/usr/bin/env python3
"""
Enhanced Gas Sensor Array System - SMART AUTO DRIFT SOLUTION + VOLTAGE ADJUSTMENT
Versi final lengkap dengan solusi untuk baseline 1.6V:
...
5. Smart Voltage Adjustment untuk sensor tidak responsif
6. Lengkap dengan semua fitur troubleshooting
"""
...
class SmartDriftManager:
    """Smart Drift Manager - Solusi lengkap untuk 1.6V baseline + Voltage Adjustment"""
    ...
    def check_sensor_responsivity(self, sensor_array, test_duration=30):
        """
        Check responsivitas semua sensor dan recommend voltage adjustment jika perlu
        """
        print(f"\n🧪 SENSOR RESPONSIVITY CHECK")
        print("="*60)
        print("Testing sensor stability dan voltage untuk detect masalah responsivitas")
        ...
        for sensor_name, config in sensor_array.sensor_config.items():
            ...
            for i in range(test_duration):
                reading = sensor_array.read_sensors()[sensor_name]
                voltages.append(reading['raw_voltage'])
                ppms.append(reading['ppm'])
                
                if i % 10 == 0:
                    print(f"  Progress: {i+1}/{test_duration} - V: {reading['raw_voltage']:.3f}V, PPM: {reading['ppm']:.1f}")
            ...
            elif voltage_mean < 1.2:
                voltage_status = "POOR"
                recommendation = "RECOMMENDED: Run Smart Voltage Adjustment"
            elif voltage_mean < 1.4:
                voltage_status = "LOW"
                recommendation = "CONSIDER: Smart Voltage Adjustment for better response"
            elif voltage_mean > 2.0:
                voltage_status = "HIGH"
                recommendation = "CONSIDER: Lower voltage to avoid over-sensitivity"
            
            responsivity_report[sensor_name] = {
            ...
            print(f"\n{sensor_name} RESULTS:")
            print(f"  Voltage: {voltage_mean:.3f}V ± {voltage_std:.3f}V ({voltage_status})")
            print(f"  PPM: {ppm_mean:.1f} ± {ppm_std:.1f}")
            print(f"  Recommendation: {recommendation}")
        
        print(f"\n" + "="*60)
        print("🧪 RESPONSIVITY CHECK SUMMARY")
        print("="*60)
        
        needs_adjustment = []
        for sensor_name, report in responsivity_report.items():
            status = report['voltage_status']
            if status in ['CRITICAL', 'POOR']:
                needs_adjustment.append(sensor_name)
                print(f"❌ {sensor_name}: {status} - {report['recommendation']}")
            elif status == 'LOW':
                print(f"⚠️ {sensor_name}: {status} - {report['recommendation']}")
            else:
                print(f"✅ {sensor_name}: {status} - Working well")
        
        if needs_adjustment:
            print(f"\n🔧 RECOMMENDED ACTIONS:")
            for sensor in needs_adjustment:
                print(f"   1. Run Smart Voltage Adjustment for {sensor}")
                print(f"   2. Target voltage: 1.6V")
                print(f"   3. Use menu option 17")
            
            run_adjustment = input(f"\nRun Smart Voltage Adjustment now? (y/n): ").lower()
            if run_adjustment == 'y':
                for sensor in needs_adjustment:
                    print(f"\n🔧 Starting adjustment for {sensor}...")
                    self.smart_voltage_adjustment(sensor_array, sensor, 1.6)
                    
                    verify = input(f"Test {sensor} responsivity with gas spray? (y/n): ").lower()
                    if verify == 'y':
                        print(f"Spray gas near {sensor} dan observe PPM change...")
                        for i in range(20):
                            reading = sensor_array.read_sensors()[sensor]
                            print(f"\r{sensor} PPM: {reading['ppm']:.1f}", end="")
                            time.sleep(1)
                        print()
        else:
            print(f"\n✅ ALL SENSORS: Voltage levels optimal")
            print(f"✅ No voltage adjustment needed")
        
        return responsivity_report
    ...
    def save_drift_data(self):
        ...
        drift_data = {
            'timestamp': datetime.now().isoformat(),
            'version': 'smart_drift_v2.0_with_voltage_adjustment',
...
class EnhancedDatasheetGasSensorArray:
    def __init__(self):
        """Initialize enhanced gas sensor array system with Smart Drift Manager + Voltage Adjustment"""
        # Setup logging
        logging.basicConfig(
            level=logging.INFO,
        ...
        # Initialize SMART Drift Manager with Voltage Adjustment
        ...
        self.logger.info("Enhanced Gas Sensor Array System with Smart Drift Manager + Voltage Adjustment initialized")
    ...
    def read_sensors(self):
        """Enhanced sensor reading with SMART drift compensation + voltage adjustment tracking"""
        readings = {}
        ...
        for sensor_name, config in self.sensor_config.items():
            try:
                ...
                readings[sensor_name] = {
                    ...
                    'normalization_factor': 1.0, 'voltage_adjusted': False
                }
    ...
    def train_model(self):
        ...
        model_metadata = {
            ...
            'model_type': 'smart_drift_compensation_v2.0_with_voltage_adjustment',
        ...
        self.logger.info("Smart drift compensation model with voltage adjustment trained and saved successfully")
...
def main():
    """Enhanced main function with Smart Drift Management + Voltage Adjustment"""
    ...
    while True:
        ...
        print("Smart Drift Compensation + Voltage Adjustment Solution")
        print("="*70)
        ...
        try:
            ...
            if choice == '1':
                ...
                print("🤖 Training model with Smart Drift compensation + Voltage adjustment features...")
                if gas_sensor.train_model():
                    print("✅ Smart model training completed successfully!")
                    print("Model includes drift correction, baseline normalization, and voltage adjustment.")
                else:
                ...
                print("\n" + "="*70)
                print("🧠 SMART SENSOR ANALYSIS - WITH DRIFT COMPENSATION + VOLTAGE ADJUSTMENT")
                print("="*70)
            ...
            # SMART DRIFT COMPENSATION OPTIONS
                ...
                for sensor_name, data in current_readings.items():
                    ...
                    print(f"  {sensor_name}: {data['raw_voltage']:.3f}V | Drift: {drift_mv:.0f}mV | {sensor_status} | Voltage: {voltage_status}")
                ...
                if status['model_compatible'] and all_good:
                    ...
                    print("⚠️ Consider AUTO BASELINE RESET (Option 15) or Voltage Adjustment (Option 17)")
                    ...
                    if test_responsivity == 'y':
                        ...
                        try:
                            while True:
                                ...
                                print(f"\r{sensor_choice}: {reading['raw_voltage']:.3f}V | PPM: {reading['ppm']:.1f}", end="")
                                time.sleep(1)
                        except KeyboardInterrupt:
                            print("\nTest completed!")
