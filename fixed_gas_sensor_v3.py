...
class OptimizedGasSensorArray:
    def __init__(self):
        ...
        # Initialize simulation mode flag first
        self.simulation_mode = not HARDWARE_AVAILABLE
        
        # Initialize I2C and ADC if hardware is available
        ...
        self.sensor_config = {
            'TGS2600': {
                'channel': getattr(self, 'tgs2600', None) if not self.simulation_mode else None,
            ...
            'TGS2602': {
                'channel': getattr(self, 'tgs2602', None) if not self.simulation_mode else None,
            ...
            'TGS2610': {
                'channel': getattr(self, 'tgs2610', None) if not self.simulation_mode else None,
        ...
        if self.simulation_mode:
            self.logger.warning("Running in SIMULATION MODE - using dummy sensor data")
    ...
    def get_simulated_voltage(self, sensor_name):
        """Generate simulated voltage values for testing"""
        import random
        base_voltages = {
            'TGS2600': 0.8 + random.uniform(-0.2, 0.2),
            'TGS2602': 0.9 + random.uniform(-0.2, 0.2),
            'TGS2610': 0.7 + random.uniform(-0.2, 0.2)
        }
        return base_voltages.get(sensor_name, 0.8)
    ...
    def read_sensors(self):
        ...
        for sensor_name, config in self.sensor_config.items():
            try:
                # Read raw voltage
                if self.simulation_mode:
                    voltage = self.get_simulated_voltage(sensor_name)
                else:
                    if config['channel'] is not None:
                        voltage = config['channel'].voltage
                    else:
                        voltage = 0.8  # Default fallback
    ...
    def calibrate_sensors(self, duration=300):
        ...
        if not self.simulation_mode:
            ...
            self.logger.info("Simulation mode: Starting calibration automatically...")
    ...
    def collect_training_data(self, gas_type, duration=60, samples_per_second=1):
        ...
        if not self.simulation_mode:
            ...
            self.logger.info(f"Simulation mode: Collecting data for {gas_type}")
        ...
        while time.time() - start_time < duration:
            ...
            print(f"\rTime: {remaining:.1f}s | "
                  ...
                  f"2610: {readings['TGS2610']['ppm']:.1f}ppm", end="")
    ...
    def load_model(self):
        """Load saved machine learning model and scaler with validation"""
        try:
            # Check if models directory exists
            if not Path('models').exists():
                self.logger.warning("Models directory not found. Please train the model first.")
                return False
            
            # Load model
            model_path = Path('models/gas_classifier.pkl')
            if model_path.exists():
                self.model = joblib.load(model_path)
                self.logger.info("Machine learning model loaded successfully")
            else:
            ...
            if scaler_path.exists():
                ...
                self.logger.warning("No scaler found. Please train the model first.")
                self.scaler = StandardScaler()  # Initialize default scaler
                return False
            ...
            if metadata_path.exists():
                ...
                self.logger.info(f"Model accuracy: {metadata.get('accuracy', 'Unknown'):.3f}")
                self.logger.info(f"Model trained on: {metadata.get('timestamp', 'Unknown')}")
...
def main():
    ...
    print("="*60)
    print("Gas Sensor Array System - USV Air Pollution Detection")
    print("Version 3.0 - All Errors Fixed")
    ...
    try:
        ...
        while True:
            print("\n" + "="*60)
            print("Enhanced Gas Sensor Array System")
            print("="*60)
            ...
            print("8. System status and info")
            print("9. Exit")
            print("-"*60)
            ...
            try:
                ...
                if choice == '1':
                    duration_input = input("Calibration duration (seconds, default 300): ").strip()
                    ...
                    duration_input = input("Collection duration (seconds, default 60): ").strip()
                    ...
                    print("SYSTEM STATUS")
                    print("="*50)
                    print(f"Simulation Mode: {'ON' if gas_sensor.simulation_mode else 'OFF'}")
                    print(f"Hardware Available: {'NO' if not HARDWARE_AVAILABLE else 'YES'}")
                    print(f"Model Trained: {'YES' if gas_sensor.is_model_trained else 'NO'}")
                    print(f"Temperature Compensation: {'ON' if gas_sensor.temp_compensation_enabled else 'OFF'}")
                    print(f"Humidity Compensation: {'ON' if gas_sensor.humidity_compensation_enabled else 'OFF'}")
                    print(f"Current Temperature: {gas_sensor.current_temperature}°C")
                    print(f"Current Humidity: {gas_sensor.current_humidity}%RH")
                    
                    # Check calibration status
                    calibrated_sensors = sum(1 for config in gas_sensor.sensor_config.values() if config['R0'])
                    print(f"Calibrated Sensors: {calibrated_sensors}/3")
                    
                    # Check data files
                    data_files = len(list(Path("data").glob("training_*.csv"))) if Path("data").exists() else 0
                    print(f"Training Data Files: {data_files}")
        ...
        print("Please check hardware connections and dependencies.")
