#!/usr/bin/env python3
"""
Enhanced Gas Sensor Array System with Drift Compensation
Solusi untuk mengatasi masalah voltage drift dan prediksi yang tidak konsisten
Version: 2.0 with Drift Compensation
"""
...
class DriftCompensationManager:
    ...
    def predict_gas(self, readings):
        """Enhanced gas prediction with drift compensation"""
    ...
    def continuous_monitoring(self, duration=None, monitoring_mode='datasheet'):
        """Enhanced continuous monitoring with drift compensation"""
        # Set monitoring mode
        self.set_sensor_mode(monitoring_mode)

        self.logger.info(f"Starting enhanced monitoring with drift compensation in {monitoring_mode.upper()} mode...")
        ...
        with open(monitoring_file, 'w', newline='') as csvfile:
            ...
            try:
                while self.is_collecting:
                    ...
                    # Enhanced data row with drift compensation
                    data_row = {
                        'timestamp': timestamp,
                    ...
                    # Enhanced display with drift compensation info
...
def main():
    """Enhanced main function with drift compensation workflow"""
    gas_sensor = EnhancedDatasheetGasSensorArrayV2()
    ...
    while True:
        print("\n" + "="*70)
        print("ENHANCED Gas Sensor Array System v2.0 - WITH DRIFT COMPENSATION")
        print("Hybrid Datasheet Approach + Automatic Drift Correction")
        print("="*70)
        print("1. Calibrate sensors (Enhanced R0 determination)")
        print("2. Collect training data (Auto-switch to Extended mode)")
        print("3. Train machine learning model (With drift features)")
        ...
        try:
            ...
            if choice == '1':
                ...
                print("✅ Drift compensation will be automatically applied to training data")
                ...
                if gas_sensor.train_model():
                    print("✅ Enhanced drift-compensated model training completed successfully!")
                ...
                print("\n" + "="*60)
                print("DETAILED SENSOR ANALYSIS - WITH DRIFT COMPENSATION")
                print("="*60)
    ...
    def get_compensation_status(self):
        ...
        for sensor_name, factor in self.drift_compensation_factors.items():
            ...
            status[sensor_name] = {
                ...
                'status': 'HIGH' if compensation_percent > 15 else 'MODERATE' if compensation_percent > 5 else 'LOW'
            }
        return status
...
class EnhancedDatasheetGasSensorArrayV2:
    """Enhanced Gas Sensor Array with Drift Compensation v2.0"""
    
    def __init__(self):
        """Initialize enhanced gas sensor array system with drift compensation"""
        # Setup logging
        logging.basicConfig(
            level=logging.INFO,
        ...
        # Enhanced sensor configurations (same as before)
        self.sensor_config = {
            'TGS2600': {
        ...
        # Initialize Drift Compensation Manager
        ...
        self.logger.info("Enhanced Gas Sensor Array System v2.0 with Drift Compensation initialized")
    ...
    def read_sensors(self):
        """Enhanced sensor reading with drift compensation"""
        readings = {}
    ...
    # Include all the original methods (calibrate_sensors, load_calibration, collect_training_data, 
    # train_model, load_training_data, load_model, predict_gas, continuous_monitoring, etc.)
    # [Methods tetap sama seperti sebelumnya, hanya read_sensors yang enhanced]
    ...
    def get_sensor_health_report(self):
        ...
        compensation_status = self.drift_manager.get_compensation_status()
        if compensation_status:
            print(f"\n📊 DRIFT COMPENSATION SUMMARY:")
            print("-" * 50)
            for sensor_name, status in compensation_status.items():
                print(f"{sensor_name}: {status['compensation_percent']:.1f}% ({status['status']} level)")
        
        # Baseline history
        if self.drift_manager.baseline_history:
            print(f"\n📈 BASELINE DRIFT HISTORY:")
            print("-" * 50)
            for sensor_name, history in self.drift_manager.baseline_history.items():
                if len(history) > 1:
                    recent_history = history[-7:]  # Last 7 entries
                    drift_range = max(recent_history) - min(recent_history)
                    drift_percent = (drift_range / recent_history[0]) * 100 if recent_history[0] > 0 else 0
                    
                    print(f"{sensor_name}: {drift_percent:.1f}% total drift over {len(recent_history)} measurements")
        ...
        if self.drift_manager.is_daily_check_needed():
            print("🔄 Daily drift check recommended - Run Menu option 11")
        
        # Check if recalibration is needed
        days_since_calibration = 0
        if self.drift_manager.last_calibration_time:
            days_since_calibration = (datetime.now() - self.drift_manager.last_calibration_time).days
            
        if days_since_calibration > 7:
            print(f"⚠️  Full recalibration recommended ({days_since_calibration} days since last calibration)")
        
        max_drift = max([abs(1 - factor) * 100 for factor in self.drift_manager.drift_compensation_factors.values()] + [0])
        
        if max_drift > 20:
            print("❌ CRITICAL: Hardware adjustment required")
            print("   Run: python3 enhanced_potentiometer_adjustment.py")
        elif max_drift > 10:
            print("⚠️  Consider hardware adjustment if accuracy is poor")
        elif max_drift > 0:
            print("✅ Drift compensation is working effectively")
        else:
            print("✅ All sensors operating normally")

    # Add other methods from original code (train_model, predict_gas, etc.)
    # [Include all remaining methods from the original main.py]
    ...
    def collect_training_data(self, gas_type, duration=60, samples_per_second=1):
        ...
        self.logger.info("Baseline readings recorded with drift compensation")
    ...
    def analyze_enhanced_training_data_quality(self, df, gas_type):
        """Enhanced analysis of training data quality with drift info"""
        self.logger.info(f"Enhanced analysis for {gas_type} with drift compensation:")
        ...
        for sensor in ['TGS2600', 'TGS2602', 'TGS2610']:
            ...
            if ppm_col in df.columns:
                ...
                # Drift analysis
                drift_data = df[drift_col] if drift_col in df.columns else pd.Series([1.0] * len(df))
                avg_drift_factor = drift_data.mean()
                drift_applied = abs(1 - avg_drift_factor) > 0.01

                self.logger.info(f"  {sensor}: PPM {ppm_mean:.0f}±{ppm_std:.0f} (range: {ppm_min:.0f}-{ppm_max:.0f})")
                
                if drift_applied:
                    drift_percent = abs(1 - avg_drift_factor) * 100
                    self.logger.info(f"  Drift compensation applied: {drift_percent:.1f}% (factor: {avg_drift_factor:.3f})")
    ...
    def train_model(self):
        ...
        model_metadata = {
            ...
            'model_type': 'enhanced_drift_compensated',
        ...
        self.is_model_trained = True
        self.logger.info("Enhanced drift-compensated model trained and saved successfully")
