...
class GasDetectionSystem:
    ...
    def load_and_train_model(self):
        ...
        try:
            ...
            # Read CSV with proper handling of spaces and semicolons
            df = pd.read_csv(self.dataset_path, sep=';', skipinitialspace=True)
            ...
            possible_names = [
                ...
                ['TGS2600', 'TGS2602', 'TGS2610'],
                ['TGS_2600', 'TGS_2602', 'TGS_2610'],
                ['tgs2600', 'tgs2602', 'tgs2610']
            ]
            ...
            if feature_columns is None:
                self.logger.error(f"Could not find sensor columns. Available: {df.columns.tolist()}")
                sys.exit(1)
            
            self.logger.info(f"Using columns: {feature_columns}")
            ...
            if label_col is None:
                self.logger.error(f"Could not find label column. Available: {df.columns.tolist()}")
                sys.exit(1)
                
            y = df[label_col].str.strip().values
